  ...
  >
    <ElForm ref="formRef" :model="form" :rules="rules" label-width="100px">
      ...
      <ElFormItem label="英文名称">
        <ElInput v-model="form.name_en" placeholder="English Name" />
      </ElFormItem>
      <ElFormItem label="越南语">
        <ElInput v-model="form.name_vi" placeholder="Tên tiếng Việt" />
      </ElFormItem>
      <ElFormItem label="泰语">
        <ElInput v-model="form.name_th" placeholder="ชื่อภาษาไทย" />
      </ElFormItem>
      <ElFormItem label="马来语">
        <ElInput v-model="form.name_ms" placeholder="Nama Melayu" />
      </ElFormItem>
      <ElFormItem label="图标">
        <div class="icon-upload-area">
          ...
          <ElInput v-model="form.icon" placeholder="或直接输入图标URL" style="margin-top: 8px;" />
        </div>
      </ElFormItem>
      ...
      <ElFormItem label="链接地址" prop="link">
        <ElInput v-model="form.link" :placeholder="form.link_type === 1 ? '如 /vip 或 /member' : '如 https://example.com'" />
      </ElFormItem>
      <ElFormItem label="排序">
...
import { createQuickMenu, updateQuickMenu, QuickMenu } from '@/api/maintenance-quick-menu'
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      if (props.type === 'add') {
        await createQuickMenu(form)
      } else {
        await updateQuickMenu(form)
      }
      visible.value = false
      emit('submit')
    } finally {
